...
require_once ROOT . 'libs/oss/OssClient.php';
require_once ROOT . 'libs/oss/Core/OssException.php';
...
class OssUpload {
    ...
    const ACCESS_KEY_ID         = 'LTAIqHZzi5imNjuu';
    const ACCESS_KEY_SECRET     = 'hBTXP6KTwjIVEtpbDI2ZEKsPUAJh1d';

    const END_POINT             = 'http://oss-accelerate.aliyuncs.com';
    ...
    protected function _mv( array $arr ) {
        ...
        // 上传文件至 Oss
        try {
            $oss_client    = new OssClient(self::ACCESS_KEY_ID, self::ACCESS_KEY_SECRET, self::END_POINT);
            $oss_file_info = $oss_client->uploadFile(self::BUCKET, $arr['http_uri'], $arr['full_path']);
        } catch (OssException $e) {
            $this->_errorInfo = $e->getMessage();
            return FALSE;
        }
        $arr['http_uri']  = $oss_file_info['oss-request-url'];
        $arr['http_url']  = $oss_file_info['oss-request-url'];
